...
def install_genome(galaxy_ref, genome, data_manager_list):
    """
    ...
    :param genome: A dictionary containing information about an individual genome to install

    :type data_manager_list: A list of data managers from the indices file
    :param data_manager_list: All data managers in this list will be installed
    """
    errored_dms = []
    dbkey_name = genome.get('dbkey')
    for idx, dm in enumerate(data_manager_list):
        dm_tool = dm.get('id')
        # Initate tool installation
        log.debug('[DM: {0}/{1}] Installing genome {2} with '
                  'Data Manager: {3}'.format(idx,
                                  len(data_manager_list), dbkey_name, dm_tool))
        tool_input = genome
...
def install_genomes(galaxy_url, api_key, indices_file):
    ...
    galaxy_ref = GalaxyInstance(galaxy_url or indices_list['galaxy_instance'], api_key or indices_list['api_key'])
    indices_list = load_indices_list(indices_file)
    for idx, genome in enumerate(indices_list['genomes']):
        log.debug('Processing {0} of {1} genomes - name: {2}'.format(idx, len(indices_list['genomes']), genome['dbkey']))
        errored_dms = install_genome(galaxy_ref, genome, indices_list['data_managers'])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-g", "--galaxy", default=DEFAULT_GALAXY_URL, help="URL of galaxy server to use. The default is %s" % DEFAULT_GALAXY_URL)
    parser.add_argument("-a", "--api_key", type=str, help="Galaxy admin user API key", required=True)
    parser.add_argument("-i", "--indices_file", type=str, help="Reference genomes to install (see genome_list.yaml.sample)", required=True)
